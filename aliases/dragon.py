...
if args:
    ...
    valid_modes = ["bite", "railgun", "flame", "rail", "wings", "recharge", "sync", "cocoon", "aegis", "sentry", "camp"]
...
if active_module == "bite":
    ...
    ch.mod_cc(cc, -1)
    title, desc = "Vyrakath: Railgun Mode", "Magnetic rails accelerate a scrap-metal slug."
    ...
    ch.mod_cc(cc, -1)
    title, desc = "Vyrakath: Flamethrower", "Igniting pressurized Fernian essence."
    ...
    ch.mod_cc(cc, -1)
    title, desc = "Vyrakath: Soarsled Wings", "Retractable soarwood panels deploy for high-mobility flight."
    fields = f'-f "Movement|Flight: 40ft (Hover)" -f "Evasion|+{pb} to AC/DEX Saves" -f "Battery|{ch.get_cc(cc)}/5"'

elif active_module == "cocoon":
    title, desc = "Vyrakath: Armored Cocoon", "Plating shifts and overlaps, forming a protective cradle for the rider."
    fields = '-f "Defense|Gero has Three-Quarters Cover (+5 AC/DEX Saves)" -f "Status|Mag-Lock Saddle Engaged"'

elif active_module in ["aegis", "sentry", "camp"]:
    cc = "Planar Battery"
    if not ch.cc_exists(cc): ch.create_cc(cc, 0, 5, "long", "bubble", None, None, 5)
    if ch.get_cc(cc) >= 2:
        ch.mod_cc(cc, -2)
        title, desc = "Vyrakath: Aegis Perimeter", "The wings unfold into a shimmering canopy filtering the Mournland mists."
        fields = f'-f "Perimeter|30ft Safe Zone" -f "Security|Advantage on Perception" -f "Battery|{ch.get_cc(cc)}/5"'
    else:
        return 'echo "PLANAR SYNC FAILURE: Insufficient battery for Aegis Filter!"'
